	...
	if (user == M && M.mind && M.mind.vampire)
		...
		if (reagents.get_reagent_amount(/datum/reagent/blood))
			...
			while (do_after(user, 30, src))
				...
				user.add_blood(blood_taken)

				if (blood_taken)
					to_chat(user, SPAN_NOTICE("You have accumulated [user.get_blood_usable()] [user.get_blood_usable() > 1 ? "units" : "unit"] of usable blood. It tastes quite stale."))

...
/mob/living/carbon/human/proc/get_medical_data_ui()
	...
	if(H.should_have_organ(BP_BRAIN))
		...
		if (!brain || H.stat == DEAD || (H.status_flags & FAKEDEATH) || ((H.status_flags & UNDEAD) && !(H.status_flags & FAKELIVING)))
	...
	data["body_temperature_c"] = H.get_body_temperature() - T0C
...
/mob/living/carbon/human/proc/get_medical_data()
	...
	// Body temperature.
	dat += "<b>Body temperature:</b> [H.get_body_temperature()-T0C]&deg;C ([H.get_body_temperature()*1.8-459.67]&deg;F)"
	if(H.nutrition < 150)
